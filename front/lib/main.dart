      ...
      try {
        ...
        if (userId > 0) {
          ...
          if (dbUser != null) {
            ...
            final bool isResellerAccount = (dbType == 1) ||
                dbRole == 'reseller' ||
                dbEmail.contains('purva') ||
                dbEmail.contains('reseller') ||
                dbName.contains('purva');

            final int currentType = isResellerAccount ? 1 : (session['userType'] ?? 0);
            ...
            session['userRole'] = isResellerAccount ? 'Reseller' : (dbUser['role'] ?? session['userRole'] ?? 'Student');
        ...
        return MaterialApp(
          ...
          home: FutureBuilder<Map<String, dynamic>>(
            ...
            builder: (context, snapshot) {
              ...
              final String userName = session?['userName'] ?? 'purva';
              final String userEmail = session?['userEmail'] ?? 'purva@gmail.com';
              final String userRole = session?['userRole'] ?? 'Reseller';
              ...
              final bool isReseller = (userType == 1) ||
                  role == 'reseller' ||
